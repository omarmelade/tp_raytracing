...
/*
 * Fill entire _color buffer with specified _color
 */
void easyppm_clear(PPM* ppm, ppmcolor c) {
    ...
    if (ppm->itype == IMAGETYPE_PBM && !easyppm_is_black_white(c))
        easyppm_abort(ppm, "Passed invalid _color to easyppm_clear() for PBM image\n");
    if (ppm->itype == IMAGETYPE_PGM && !easyppm_is_grey(c))
        easyppm_abort(ppm, "Passed invalid _color to easyppm_clear() for PGM image\n");
...
/*
 * Set appropriate _color (RGB or greyscale)
 */
...
/*
 * Get appropriate _color (RGB or greyscale)
 */
...
/*
 * Create _color from 8-bit RGB values
 */
...
/*
 * Create _color from 8-bit greyscale value
 */
...
/*
 * Create _color from binary value: 0 for white
...
/*
 * Invert image y-axis for applications that assume an _origin
 * in the lower left corner (easyppm defaults to an _origin in
...
/*
 * Determine if _color is greyscale
 */
static int easyppm_is_grey(ppmcolor c) {
...
/*
 * Determine if _color is black or white
 */
static int easyppm_is_black_white(ppmcolor c) {
